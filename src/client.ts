...
export interface ClientOptions {
  /**
   * Defaults to process.env['BEM_SDK_API_KEY'].
   */
   ...
   * - `production` corresponds to `https://api.bem.ai`
   * - `sandbox` corresponds to `https://sandbox-api.bem.ai`
   */
   ...
   *
   * Defaults to process.env['BEM_SDK_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['BEM_SDK_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Bem SDK API.
 */
export class BemSDK {
  apiKey: string;
   ...
   * API Client for interfacing with the Bem SDK API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['BEM_SDK_API_KEY'] ?? undefined]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env['BEM_SDK_BASE_URL'] ?? https://api.bem.ai] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('BEM_SDK_BASE_URL'),
    apiKey = readEnv('BEM_SDK_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.BemSDKError(
        "The BEM_SDK_API_KEY environment variable is missing or empty; either provide it, or instantiate the BemSDK client with an apiKey option, like new BemSDK({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? BemSDK.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('BEM_SDK_LOG'), "process.env['BEM_SDK_LOG']", this) ??
      defaultLogLevel;
  ...
  static BemSDKError = Errors.BemSDKError;
